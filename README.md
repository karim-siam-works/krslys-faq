...
- Upload the `all-in-one-faq` folder to the `wp-content/plugins` directory.
- Activate the plugin through the “Plugins” page in WordPress.
- Go to `Settings → All-in-One FAQ` to configure your FAQ styles.

### Usage

1. Configure your global FAQ style in the admin style page (`Settings → All-in-One FAQ`).
...
3. Add the shortcode `[aio_faq]` inside any post, page, or widget area to render all published FAQs using your saved styles.
...
- A basic `block.json` is included for an `all-in-one-faq/faq` block so that a future editor script can reuse the same styling.
